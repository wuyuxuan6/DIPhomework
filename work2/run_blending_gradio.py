...
def create_mask_from_points(points, img_h, img_w):
    ...
    draw.polygon([tuple(point) for point in points.tolist()], outline=255, fill=255)
...
def cal_laplacian_loss(foreground_img, foreground_mask, blended_img, background_mask):
    ...
    foreground_laplacian = F.conv2d(foreground_img, laplacian_kernel, padding=1, groups=channels)
    blended_laplacian = F.conv2d(blended_img, laplacian_kernel, padding=1, groups=channels)

    foreground_values = foreground_laplacian[foreground_mask.bool().expand(-1, channels, -1, -1)]
    blended_values = blended_laplacian[background_mask.bool().expand(-1, channels, -1, -1)]
